...
@property (assign ,nonatomic)BOOL       sengImage;
...
- (IBAction)send:(id)sender {
    ...
    [self.manager sendData:[self.message.text dataUsingEncoding:NSUTF8StringEncoding]
                     topic:[NSString stringWithFormat:@"%@/%@-%@",
                       ...
                       qos:MQTTQosLevelAtLeastOnce
                    retain:FALSE];
    NSLog(@"uidevice %@ send data %@ at topic %@ ",[UIDevice currentDevice].name,self.message.text,[NSString stringWithFormat:@"%@/%@-%@",
                                                                                                    self.base,
                                                                                                    [UIDevice currentDevice].name,
                                                                                                    self.tabBarItem.title]);
    
    
    if ([self.message.text isEqualToString:@"chat"]) {
        
        self.sengImage = YES;
        
        NSString *filePath = [[NSBundle mainBundle] pathForResource:@"chat" ofType:@"png"];
        
        NSData *imageData = [NSData dataWithContentsOfFile:filePath];
        
        [self.manager sendData:imageData topic:[NSString stringWithFormat:@"%@/%@-%@",
                                                self.base,
                                                [UIDevice currentDevice].name,
                                                self.tabBarItem.title] qos:MQTTQosLevelAtLeastOnce retain:FALSE];
    }
...
- (void)handleMessage:(NSData *)data onTopic:(NSString *)topic retained:(BOOL)retained {
    ...
    if (self.sengImage) {
        ...
        self.chatImage.image = chatImage;
    }
    
    
    NSLog(@"receive %@ send %@",senderString,dataString);
